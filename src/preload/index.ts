...
const api: TamepadApi = {
  ...
  setSnoozed: (snoozed: boolean) =>
    ipcRenderer.invoke(IpcChannel.WindowSetSnoozed, snoozed) as Promise<void>,
  ...
  onSnoozeChanged: (cb) => {
    ipcRenderer.on(IpcChannel.SnoozeChanged, (_event, snoozed: boolean) => {
      cb(snoozed);
    });
  },
  notifyReady: () => {
